...
function generateMockups(siteName) {
  ...
  Object.entries(themes).forEach(([themeKey, themeData]) => {
    ...
    console.log(`✅ Generated ${themeData.name} theme: mockups/${siteName}/config-${themeKey}.json`);
  });
...
`;
    ...
    console.log(`✅ Generated Tailwind config: mockups/${siteName}/tailwind.${themeKey}.mjs`);
  });

  console.log(`\n🎉 Generated ${Object.keys(themes).length} theme variations!`);
  console.log(`\n📝 To preview a theme:`);
  console.log(`   1. Copy mockups/${siteName}/tailwind.<theme>.mjs to tailwind.config.mjs`);
